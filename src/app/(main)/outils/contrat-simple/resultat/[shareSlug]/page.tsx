...
export const metadata: Metadata = {
  title: "Ton contrat est prêt",
  robots: { index: false },
};

// Page publique — aucune authentification, aucun quota requis pour
// consulter un contrat déjà généré (même principe que les autres outils PDF).
export default async function ContratResultPage({
  params,
}: {
  ...
  const pdfUrl = `/api/outils/contrat-simple/${shareSlug}`;
  const resultUrl = `${siteConfig.url}/outils/contrat-simple/resultat/${shareSlug}`;
  const whatsappMessage = encodeURIComponent(`Voici le contrat que j'ai préparé : ${resultUrl}`);
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-12 sm:px-6 sm:py-16">
      <div className="mb-8 flex flex-col gap-3 text-center sm:text-left">
        ...
        <h1 className="text-3xl font-bold tracking-tight sm:text-4xl">Ton contrat est prêt</h1>
        <p className="text-lg text-muted-foreground">
          Télécharge-le en PDF, ou partage-le — pense à le faire relire avant signature.
        </p>
      </div>
