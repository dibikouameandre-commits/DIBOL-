...
export function CreatifFactureHtmlPreview({ data }: { data: FactureResultData }) {
  ...
  return (
    <div className="bg-white text-neutral-900" style={{ fontSize: 13 + scale.bodyFontDelta }}>
      <div
        className="flex items-center justify-between gap-6 px-9 py-6"
      ...
      >
        <div className="max-w-[60%]">
          ...
          <h2 className="text-xl font-bold text-white">{form.issuerName || "Nom de l'entreprise"}</h2>
          {form.issuerAddress && (
            <p className="whitespace-pre-line text-xs text-white/90">{form.issuerAddress}</p>
          )}
          ...
          {form.issuerTaxId && (
            <p className="text-xs text-white/90">RCCM / NIF : {form.issuerTaxId}</p>
          )}
        </div>

        <div className="shrink-0 text-right">
          <span
            className="inline-block rounded px-3.5 py-1.5 text-sm font-bold tracking-wide"
            style={{ backgroundColor: "#FFFFFF", color: meta.accent }}
          >
            {docLabel}
          </span>
          <div className="mt-2 flex flex-col gap-0.5 text-xs text-white">
            ...
            {form.dueDate && (
              <p>
                <span className="text-white/75">Échéance </span>
      ...
      <div className="px-9 pb-9">
        <div className="max-w-[60%]" style={{ marginBottom: 22 * scale.sectionGapMultiplier }}>
          <p
        ...
        <div className="overflow-hidden rounded">
          <div
            className="flex gap-3 px-2.5 py-2 text-[10px] font-bold tracking-wide text-white uppercase"
          ...
          {totals.lines.map((line, i) => (
            <div
              key={i}
              className="flex items-baseline gap-3 px-2.5 text-xs"
              style={{
                backgroundColor: i % 2 === 1 ? meta.accentSoft : "transparent",
        ...
        >
          ...
          <div
            className="mt-1 flex justify-between rounded px-2.5 py-2 text-sm font-bold text-white"
