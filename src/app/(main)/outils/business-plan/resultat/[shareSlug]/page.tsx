...
import { BusinessPlanPreview } from "../../business-plan-preview";

export const metadata: Metadata = {
  title: "Ton business plan est prêt",
  robots: { index: false },
};

// Page publique — aucune authentification, aucun quota requis pour
// consulter un business plan déjà généré (même principe que le CV/la
// lettre/la lettre administrative).
export default async function BusinessPlanResultPage({
  params,
}: {
  ...
  const pdfUrl = `/api/outils/business-plan/${shareSlug}`;
  const resultUrl = `${siteConfig.url}/outils/business-plan/resultat/${shareSlug}`;
  const whatsappMessage = encodeURIComponent(
    `Voici le business plan que j'ai préparé : ${resultUrl}`
  );
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-12 sm:px-6 sm:py-16">
      <div className="mb-8 flex flex-col gap-3 text-center sm:text-left">
        ...
        <h1 className="text-3xl font-bold tracking-tight sm:text-4xl">Ton business plan est prêt</h1>
      ...
      <div className="mx-auto w-full overflow-hidden rounded-lg border shadow-lg" style={{ maxWidth: 700 }}>
        <BusinessPlanPreview plan={result} />
      </div>
