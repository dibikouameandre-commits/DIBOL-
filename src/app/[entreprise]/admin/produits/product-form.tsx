...
import { createCompanyProduct, updateCompanyProduct } from "@/server/company-admin/products";
...
export function CompanyProductForm({
  ...
  const action = async (_prev: FormState, formData: FormData): Promise<FormState> => {
    if (isEdit) {
      return updateCompanyProduct(entreprise, product.id, formData);
    }
    return createCompanyProduct(entreprise, formData);
  };
